...
wheelOptions.chassisConnectionPointLocal.set(0, -0.1, 0.6);
...
function handleControls() {
  ...
  if (keys['a'] || keys['arrowleft']) vehicle.setSteeringValue(maxSteerVal, 0);
  else if (keys['d'] || keys['arrowright']) vehicle.setSteeringValue(-maxSteerVal, 0);

  if (keys['w'] || keys['arrowup']) vehicle.applyEngineForce(-maxForce, 1);
  else if (keys['s'] || keys['arrowdown']) vehicle.applyEngineForce(maxForce, 1);

  if (keys[' ']) { vehicle.setBrake(brakeForce, 0); vehicle.setBrake(brakeForce, 1); }
